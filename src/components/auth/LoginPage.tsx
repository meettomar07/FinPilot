      ...
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-6xl items-center gap-8 lg:grid lg:grid-cols-[1.1fr_480px]">
        <div className="hidden rounded-[2rem] border border-border bg-gradient-to-br from-[#0D47A1] via-[#1A73E8] to-[#4F8DF6] p-10 text-white shadow-2xl lg:block">
          ...
          <h1 className="mt-6 text-5xl font-bold tracking-tight" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            Your money data stays in your backend. Your identity stays with Firebase.
          </h1>
          <p className="mt-5 max-w-xl text-base leading-7 text-white/85">
            Sign in with Google or email/password to unlock your existing dashboard, forecasts, decisions, and AI assistant with real user identity.
          </p>
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {[
              "Google Sign-In and email/password auth",
              "Backend-verified Firebase ID tokens",
              "Financial data stored only in your backend database",
              "Gemini sees summary metrics only",
            ].map((item) => (
              <div key={item} className="rounded-2xl bg-white/10 p-4 text-sm leading-6 text-white/90 backdrop-blur-sm">
                {item}
              </div>
            ))}
          </div>
